<!doctype html>
<html class="no-js" lang="">
<head>
    <meta charset="utf-8">
    <meta http-equiv="x-ua-compatible" content="ie=edge">
    <title>Default Page</title>
    <meta name="description" content="">
    <meta name="viewport" content="width=device-width, initial-scale=1">

    <link href="main.css?v=<? echo time() ?>" rel="stylesheet">

</head>
<body>
    <article>
        <br>
        <br>
        <br>
        <div class="my-center-container-title">
            <h1>Web Project</h1>
        </div>
        <hr>

        <div class="my-wrapper-container">
            <p>
                This is another
                <a href="/todo.php">ToDo</a> app example.
            </p>
            <br>
            <p>
                Unlike the usual demos, in this example we are not going to use something new and awesome, we are going
                to
                re-invent the wheel, as per <span class="my-bold"><a
                            href="https://github.com/WebToLearn/laborator-tehnici-web">requirements</a></span>.
            </p>

            <br>
            <br>
            <p>
                In this project, only plain css and vanilla js will be used.
            </p>
            <br>
            <br>
            <br>
            <br>
            <br>
            <br>

            <div class="my-right-float">
                <p> Costin Costache</p>
                <p> Group 353</p>
            </div>
        </div>
    </article>
</body>
</html>
